...
# extract array information into variables
zip=${information[0]}
...
do
    ...
    # if a directory submitted, copy it to the checked folder
    if [[ -d $submission ]]; then
        remarks="issue case #1"
        deducted_marks=$penalty_submission
        total_marks_csv=-$penalty_submission
        cp -r $submission ./output/submissions/checked
    else
        # if a zip file submitted, unzip it to the checked folder
        ext_exist=0
        ...
        # if a file submitted, copy it to the checked folder inside a folder named as the student id 
            ...
            folder=$filename
        # if a file with unknown extension submitted, copy it to the issues folder
        else
            deducted_marks=$penalty_submission
            total_marks_csv=-$penalty_submission
            remarks="issue case #2"
            cp $submission ./output/submissions/issues
            echo "$(basename $submission),0,$deducted_marks,$total_marks_csv,$remarks" >> ./output/submissions/marks.csv
            has_issue=1
        fi
    fi
    ...
    # check if valid programming language used or not
    extension_match=0
    ...
    if [[ $extension_match -eq 0 ]]; then
        ...
        echo "$folder,0,$deducted_marks,$total_marks_csv,$remarks" >> ./output/submissions/marks.csv
        has_issue=1
    fi

    if [[ $has_issue -eq 0 ]]; then
        # check if the file name is the same as the folder name
        if [[ $file_name != $folder  ]]; then
            remarks="issue case #4"
            deducted_marks=$penalty_submission
        fi
        
        # run the code and create output file
        ...
        total_marks_csv=$((marks+total_marks_csv))
